...
import { parseScore } from '../score-input.ts';
...
export function createScoreRouter(pool: ScoreDatabase, { limit = 10, windowMs = 60000, now = Date.now } = {}) {
    ...
    router.post('/', async (req, res, next) => {
        ...
        const input = parseScore(req.body);
        if (!input) return res.status(400).json({ error: 'Invalid score data' });
        try {
            await pool.query('INSERT INTO scores (game_id, player_id, score) VALUES ($1, $2, $3)', ['robohorse-v1', input.name, input.score]);
